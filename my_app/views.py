...
def month_bills(request):
    ...
    open_bill_date = datetime.datetime.now()
    if bills:
...
def delete_user(request, rmn):
    ...
    try:
        ...
        messages.success(request, 'ผู้เช่า {} ห้อง {} xได้ถูกลบออกจากระบบแล้ว !!!'.format(user, rmn))
...
def maintenance_charge(request):
    if request.method == 'POST':
        ...
        if maintenance_form.is_valid():
            ...
            messages.success(request, 'ค่าบริการ ได้ถูกคิดรวมเป็นค่าใช้จ่าย ห้อง {} เรียบร้อยแล้ว'.format(rmn))
